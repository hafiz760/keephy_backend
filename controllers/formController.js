...
const getAllForms = async (req, res, next) => {
  try {
    ...
    if (!req.query?.page) {
      const forms = await Form.find({user: req.userId, isTrue: true});
      res.status(200).json({
        status: 'success',
      ...
      const filter = { isTrue: true, user: req.userId };
...
const getFormByBusinessId = async (req, res, next) => {
  try {
    const page = parseInt(req?.query?.page) || 1;
    const business = await Business.findOne({_id: req.params.id, isTrue: true});
...
const addFormSubmission = async (req, res, next) => {
  try {
    ...
    const getFormById = await Form.findById(formId);
    if (!getFormById) {
...
const deleteForm = async (req, res, next) => {
  try {
    const getFormById = await Form.findById(req.params.id);
    if (!getFormById) {
      return next(new HttpError('no Form found by this Id', 404));
    }
    getFormById.isTrue = false;
    getFormById.save();
...
const getFormById = factory.getOne(Form);
const updateFormById = factory.updateOne(Form);

const getFormByCode = async (req, res, next) => {
  try {
    console.log(req.params);
    let moduleName = 'business';
    let record = await Business.findOne({'forms.code': req.params.code, isTrue: true}).populate('forms.form');
    if (!record) {
      // find in locations
    ...
    if (!record) {
      return next(new HttpError('No Form found by this code', 404));
    }
    // find the form
