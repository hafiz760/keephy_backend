...
const getAllBusiness = async (req, res, next) => {
  try {
    ...
    if (!req.query?.page) {
      ...
      const filter = { userId: req.userId, isTrue: true };
...
const getOneBusiness = async (req, res, next) => {
  try {
    const singleBusiness = await Business.findById(req.params.id).populate([
      { path: 'categoryId' },
      { path: 'reviews' },
    ]);
...
const deleteBusiness = async (req, res, next) => {
  try {
    const getBusinessById = await Business.findById(req.params.id);
    if (!getBusinessById) {
      return next(new HttpError('no business found by this Id', 404));
    }
    getBusinessById.isTrue = false;
